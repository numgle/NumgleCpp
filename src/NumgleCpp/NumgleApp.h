...
class NumgleApp {
public:
    NumgleApp(std::unique_ptr<DatasetRepository> repo) : repo(std::move(repo)) {
    }
...
private:
    ...
    void convertOneChar(std::ostringstream& stream, uint32_t code, const ConvertTables& tables) {
        ...
        case LetterType::CompleteHangul: {
            ...
            stream << tables.jongTable.at(seperatedHan.jong);
            stream << tables.cjTable.at(std::min((uint32_t)8, seperatedHan.jung)).at(seperatedHan.cho);
            break;
        }
    ...
    static LetterType getLetterType(uint32_t letter) {
        ...
        else if (SPECIAL_CHARATERS.find(letter) != SPECIAL_CHARATERS.end()) return LetterType::SpecialLetter;
